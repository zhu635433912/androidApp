...
public class ContactsFragment extends BaseFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        List<AddFriendEntity> list = User_id.getInstance().getFriends();
        if (list.size()>0) {
            for (int i = 0; i < list.size(); i++) {
                getFriend(list.get(i));
            }
            User_id.getInstance().deleteFriend();
        }
    }
...
//    @Subscriber(tag = "friend")
    public void getFriend(AddFriendEntity entity) {
        final UserEntry user = JGApplication.getUserEntry();
        final String reason = entity.getReason();
        final String username = entity.getUsername();
        final String appKey = entity.getAppKey();
...
//    @Subscriber(tag = "add")
//    public void getAdd(Event event){
